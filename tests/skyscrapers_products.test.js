skyscrapersProductsTestSuite = testSuite("Product Skyscrapers",
...
beforeSuite((suite, cb)=> {
  ...
  requirejs(['puzzle_types/util.js', 'squarepuzzle'], function() {
    ...
    requirejs(['puzzle_types/sudoku_util.js'], function() {
      ...
      requirejs.undef('puzzle_types/skyscrapers_products.js');
      requirejs(['puzzle_types/skyscrapers_products.js'], cb);
    });
  });
}),
...
test('Solver controllers',(suite) => {
  let puzzle = suite.showPuzzle(
    "skyscrapers_products", "4x4",
    {"a1": "1", "bottom": [null,null,"12",null], "right": ["12",null,"4",null], "top": ["24",null,null,null], "left": [null,"24",null,null]}
  );
  puzzle.start();
  ...
  assert("Bottom clue click").that(puzzle.bottom[2].clickSwitch).containsExactly([{text: '12'}, {text: '12', image: 'white_circle'}]);
  ...
  assert("Left clue click").that(puzzle.left[1].clickSwitch).containsExactly([{text: '24'}, {text: '24', image: 'white_circle'}]);
...
test('Author controllers',(suite) => {
  let puzzle = suite.showPuzzle("skyscrapers_products", "4x4");
  puzzle.edit();
  ...
  assert("Bottom clue chooser").that(puzzle.bottom[0].chooserValues).containsAtLeast([{},{text: '1', returnValue: '1'},{text: '2', returnValue: '2'},{text: '3', returnValue: '3'},{text: '4', returnValue: '4'},{text: '24', returnValue: '24'}]);
  assert("Bottom clue click").that(puzzle.bottom[0].clickSwitch).isNull();
  assert("Right clue chooser").that(puzzle.right[0].chooserValues).containsAtLeast([{},{text: '1', returnValue: '1'},{text: '2', returnValue: '2'},{text: '3', returnValue: '3'},{text: '4', returnValue: '4'},{text: '24', returnValue: '24'}]);
  assert("Right clue click").that(puzzle.right[0].clickSwitch).isNull();
  assert("Top clue chooser").that(puzzle.top[0].chooserValues).containsAtLeast([{},{text: '1', returnValue: '1'},{text: '2', returnValue: '2'},{text: '3', returnValue: '3'},{text: '4', returnValue: '4'},{text: '24', returnValue: '24'}]);
  assert("Top clue click").that(puzzle.top[0].clickSwitch).isNull();
  assert("Left clue chooser").that(puzzle.left[0].chooserValues).containsAtLeast([{},{text: '1', returnValue: '1'},{text: '2', returnValue: '2'},{text: '3', returnValue: '3'},{text: '4', returnValue: '4'},{text: '24', returnValue: '24'}]);
...
test('Process click to empty cell',(suite) => {
  let puzzle = suite.showPuzzle(
    "skyscrapers_products", "4x4",
    {"bottom": [null,null,"12",null], "right": ["12",null,"4",null], "top": ["24",null,null,null], "left": [null,"24",null,null]}
  );
  puzzle.start();
...
test('Process click to top clue',(suite) => {
  ...
  assert("Cell data before click").that(puzzle.top[0].data).isEqualTo({text: "24"});

  puzzle.controller.onMouseDown(suite.mouseEvent(x, y));
  puzzle.controller.onMouseUp(suite.mouseEvent(x, y));

  assert("Cell value after one click").that(puzzle.top[0].getValue()).isNull();
  assert("Cell data after one click").that(puzzle.top[0].data).isEqualTo({text: "24", image:"white_circle"});
}),
...
test('Correct solution',(suite) => {
  let clues = {"bottom": [null,null,"12",null], "right": ["12",null,"4",null], "top": ["24",null,null,null], "left": [null,"24",null,null]};
...
test('Wrong clue',(suite) => {
  ...
  assert("Wrong clue solution response").that(Checker.check("4x4", clues, data)).isEqualTo({status:"Wrong product of visible buldings in the row",errors:["c4","c3","c2","c1"]});
}),
);
