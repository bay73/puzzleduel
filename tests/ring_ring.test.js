ringringTestSuite = testSuite("Ring-Ring",
...
test('Process drag on connector',(suite) => {
  let puzzle = suite.showPuzzle("ring_ring", "4x4",{"a4": "black", "d1": "black"});
  puzzle.start();
  ...
  assert("Connector data after drag").that(puzzle.connectors[0][0]['h'].data).isEqualTo({color: puzzle.colorSchema.textColor, returnValue:1});
