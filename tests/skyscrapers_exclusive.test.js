skyscrapersExclusiveTestSuite = testSuite("Exclusive Skyscrapers",
...
beforeSuite((suite, cb)=> {
  ...
  requirejs(['puzzle_types/util.js', 'squarepuzzle'], function() {
    ...
    requirejs(['puzzle_types/sudoku_util.js'], function() {
      ...
      requirejs.undef('puzzle_types/skyscrapers_exclusive.js');
      requirejs(['puzzle_types/skyscrapers_exclusive.js'], cb);
    });
  });
}),
...
test('Solver controllers',(suite) => {
  let puzzle = suite.showPuzzle(
    "skyscrapers_exclusive", "4x4",
    {"a1": "1", "bottom": [null,null,"2",null], "right": ["2",null,"1",null], "top": ["4",null,null,null], "left": [null,"3",null,null]}
  );
  puzzle.start();

  assert("Empty cell chooser").that(puzzle.cells[2][2].chooserValues).containsExactly([{},{text: '1', returnValue: '1'},{text: '2', returnValue: '2'},{text: '3', returnValue: '3'},{text: '4', returnValue: '4'},{image: 'odd', keepPencil: true},{image: 'even', keepPencil: true}]);
  ...
  assert("Bottom clue click").that(puzzle.bottom[2].clickSwitch).containsExactly([{text: '2'}, {text: '2', image: 'white_circle'}, {text: '2', image: 'cross'}]);
  assert("Right clue chooser").that(puzzle.right[0].chooserValues).isNull();
  assert("Right clue click").that(puzzle.right[0].clickSwitch).containsExactly([{text: '2'}, {text: '2', image: 'white_circle'}, {text: '2', image: 'cross'}]);
  assert("Top clue chooser").that(puzzle.top[3].chooserValues).isNull();
  assert("Top clue click").that(puzzle.top[3].clickSwitch).containsExactly([{}, {image: 'white_circle'}, {image: 'cross'}]);
  assert("Left clue chooser").that(puzzle.left[1].chooserValues).isNull();
  assert("Left clue click").that(puzzle.left[1].clickSwitch).containsExactly([{text: '3'}, {text: '3', image: 'white_circle'}, {text: '3', image: 'cross'}]);
...
test('Author controllers',(suite) => {
  let puzzle = suite.showPuzzle("skyscrapers_exclusive", "4x4");
  puzzle.edit();
  ...
  assert("Top clue chooser").that(puzzle.top[0].chooserValues).containsExactly([{},{text: '0', returnValue: '0'},{text: '1', returnValue: '1'},{text: '2', returnValue: '2'}]);
  assert("Top clue click").that(puzzle.top[0].clickSwitch).isNull();
  assert("Left clue chooser").that(puzzle.left[0].chooserValues).containsExactly([{},{text: '0', returnValue: '0'},{text: '1', returnValue: '1'},{text: '2', returnValue: '2'}]);
...
test('Correct solution',(suite) => {
  let clues = {"bottom": [null,"2",null,"0"], "right": [null,"2",null,null], "top": [null,null,"2","2"], "left": [null,"2",null,"2"]};
  let data = {"a1": "3", "a2": "1", "a3": "4", "a4": "2", "b1": "4", "b2": "3", "b3": "2", "b4": "1", "c1": "2", "c2": "4","c3": "1", "c4": "3", "d1": "1", "d2": "2", "d3": "3", "d4": "4"}
...
test('Repeated digits',(suite) => {
  let clues = {"bottom": [null,"2",null,"0"], "right": [null,"2",null,null], "top": [null,null,"2","2"], "left": [null,"2",null,"2"]};
  let data = {"a1": "3", "a2": "1", "a3": "4", "a4": "2", "b1": "4", "b2": "3", "b3": "2", "b4": "1", "c1": "2", "c2": "4","c3": "1", "c4": "3", "d1": "2", "d2": "2", "d3": "3", "d4": "4"}
...
test('Wrong clue',(suite) => {
  ...
  assert("Wrong clue solution response").that(Checker.check("4x4", clues, data)).isEqualTo({status:"Wrong number of visible buldings in the row",errors:["d1","d2","d3","d4"]});
}),
);
