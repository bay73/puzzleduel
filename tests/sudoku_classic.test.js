sudokuClassicTestSuite = testSuite("Sudoku",
...
beforeSuite((suite, cb)=> {
  ...
  requirejs(['puzzle_types/util.js', 'sudokupuzzle'], function() {
    ...
    requirejs(['puzzle_types/sudoku_util.js'], function() {
      ...
      requirejs.undef('puzzle_types/sudoku_classic.js');
      requirejs(['puzzle_types/sudoku_classic.js'], cb);
    });
  });
}),
...
test('Solver controllers',(suite) => {
  let puzzle = suite.showPuzzle("sudoku_classic", "4x4",{"a1": "1", "d2":"2","b3":"3"});
  puzzle.start();
...
test('Author controllers',(suite) => {
  let puzzle = suite.showPuzzle("sudoku_classic", "4x4");
  puzzle.edit();
...
test('Process click to empty cell',(suite) => {
  let puzzle = suite.showPuzzle("sudoku_classic", "4x4",{"b2": "4", "d2":"2","b3":"3"});
  puzzle.start();
...
test('Repeated digits in diagonal',(suite) => {
  ...
  assert("Repeated digits in diagonal solution response").that(Checker.check("4x4", clues, data)).isEqualTo({status: "OK"});
}),
