fenceTurningTestSuite = testSuite("Turning Fences",
...
test('Process click on edge',(suite) => {
  let puzzle = suite.showPuzzle("fence_turning", "4x4",{"b1": "0", "a2": "3"});
  puzzle.start();
...
test('Process click on node',(suite) => {
  let puzzle = suite.showPuzzle("fence_turning", "4x4",{"b1": "0", "a2": "3"});
  puzzle.start();
  ...
  assert("Node value before click").that(puzzle.nodes[0][0][1].getValue()).isNull();
  assert("Node data before click").that(puzzle.nodes[0][0][1].data).isEqualTo({});

  puzzle.controller.onMouseDown(suite.mouseEvent(x, y));
  puzzle.controller.onMouseUp(suite.mouseEvent(x, y));

  assert("Node value after one click").that(puzzle.nodes[0][0][1].getValue()).isNull();
  assert("Node data after one click").that(puzzle.nodes[0][0][1].data).isEqualTo({image: "cross"});

  puzzle.controller.onMouseDown(suite.mouseEvent(x, y));
  puzzle.controller.onMouseUp(suite.mouseEvent(x, y));

  assert("Node value after two clicks").that(puzzle.nodes[0][0][1].getValue()).isNull();
  assert("Node data after two clicks").that(puzzle.nodes[0][0][1].data).isEqualTo({image: "white_circle"});

  puzzle.controller.onMouseDown(suite.mouseEvent(x, y));
  puzzle.controller.onMouseUp(suite.mouseEvent(x, y));

  assert("Node value after three clicks").that(puzzle.nodes[0][0][1].getValue()).isNull();
  assert("Node data after three clicks").that(puzzle.nodes[0][0][1].data).isEqualTo({});
}),
...
test('Data collector',(suite) => {
  let puzzle = suite.showPuzzle("fence_turning", "4x4",{"a1": "3", "b2": "3", "c1": "0", "d2": "1"});
  puzzle.start();
