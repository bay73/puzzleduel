sudokuAntidiagonalTestSuite = testSuite("Antidiagonal Sudoku",
...
beforeSuite((suite, cb)=> {
  ...
  requirejs(['puzzle_types/util.js', 'sudokupuzzle'], function() {
    ...
    requirejs(['puzzle_types/sudoku_util.js'], function() {
      ...
      requirejs.undef('puzzle_types/sudoku_antidiagonal.js');
      requirejs(['puzzle_types/sudoku_antidiagonal.js'], cb);
    });
  });
}),
...
test('Solver controllers',(suite) => {
  let puzzle = suite.showPuzzle("sudoku_antidiagonal", "4x4",{"a1": "1", "d2":"2","b3":"3"});
  puzzle.start();
...
test('Author controllers',(suite) => {
  let puzzle = suite.showPuzzle("sudoku_antidiagonal", "4x4");
  puzzle.edit();
...
test('Process click to empty cell',(suite) => {
  let puzzle = suite.showPuzzle("sudoku_antidiagonal", "4x4",{"b2": "4", "d2":"2","b3":"3"});
  puzzle.start();
