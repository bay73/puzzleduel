...
// В дополнение к стандартным правилам 
// на пересечении линий сетки может быть указано число рингов, окружающих данную точку.
// Пока не добавлял (размышляю о полезности) -- возможность задания букв x,y,z,n,m,k,... 
//   Число неизвестно, но одинаковые буквы -- одинаковые числа, разные -- разные.
const Checker = {
check:function(dimension, clueData, data){
  ...
  var depths =  util.create2DArrayExtended(dim.rows, dim.cols, "")
  ...
  var res = Checker.checkDepths(rectangles, depths);
  if (res.status != "OK") {
    return res;
  }
return {status: "OK"};
...
checkDepths:function (rectangles, clue) {
  for (var x = -1; x < clue.cols; x++) {
    for (var y = -1; y < clue.rows; y++) {
      if (clue[y][x]) {
        var depth = rectangles.reduce((depth,r) => depth + (r.x0 <= x && r.x2 > x && r.y0 <= y && r.y2 > y ? 1 : 0), 0);
        if (depth.toString() != clue[y][x]) {
