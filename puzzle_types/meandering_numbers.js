...
checkAreas: function(areas, cells) {
  for (var i = 0; i < areas.length; i++) {
    ...
    for (var j = 1; j <= area.length; j++) {
      values[j] = {exists: false, pos: null};
    }
    for (var j = 0; j < area.length; j++) {
      var pos = util.parseCoord(area[j]);
      var value = parseInt(cells[pos.y][pos.x]);
      if (value) {
        values[value].exists = true;
        values[value].pos = pos;
      }
    }
    for (var j = 1; j <= area.length; j++) {
      if (!values[j].exists) {
      ...
      if (j>1){
      	if (!Checker.isAdjascent(values[j].pos, values[j-1].pos)) {
        	return {status: "Consecutive numbers should share an edge in the area", errors: [util.coord(values[j].pos.x, values[j].pos.y), util.coord(values[j-1].pos.x, values[j-1].pos.y)]};
        }
      }
    }
  }
...
isAdjascent: function(pos1, pos2) {
  if (pos1.x == pos2.x && pos1.y == pos2.y-1) return true;
  if (pos1.x == pos2.x && pos1.y == pos2.y+1) return true;
  if (pos1.x == pos2.x-1 && pos1.y == pos2.y) return true;
  if (pos1.x == pos2.x+1 && pos1.y == pos2.y) return true;
  return false;
},
