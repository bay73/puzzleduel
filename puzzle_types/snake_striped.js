...
check:function(dimension, clues, data){
  ...
  var res = Checker.checkColumnClues(cells, bottom, (head.x+head.y+1)%2);
  if (res.status != "OK") {
    return res;
  }
  var res = Checker.checkRowClues(cells, right, (head.x+head.y)%2);
  if (res.status != "OK") {
    return res;
  }
  return {status: "OK"};
...
checkColumnClues: function(cells, clues, even) {
  for (var x=0; x < cells.cols; x++) {
    if (clues[x] && clues[x] != "white") {
      res = Checker.checkColumnClue(cells, clues[x], x, even);
      if (res) {
...
checkColumnClue: function(cells, clue, col, even) {
  var blackCount = 0;
  var cellList = [];
  for (var y=0; y < cells.rows; y++) {
    cellList.push(util.coord(col, y));
    if (cells[y][col] && (y+col)%2==even) blackCount++;
  }
  if (blackCount.toString() != clue) return cellList;
  return null;
},
checkRowClues: function(cells, clues, even) {
  for (var y=0; y < cells.rows; y++) {
    if (clues[y] && clues[y] != "white") {
      res = Checker.checkRowClue(cells, clues[y], y, even);
      if (res) {
...
checkRowClue: function(cells, clue, row, even) {
  var blackCount = 0;
  var cellList = [];
  for (var x=0; x < cells.cols; x++) {
    cellList.push(util.coord(x, row));
    if (cells[row][x] && (row+x)%2==even) blackCount++;
  }
  if (blackCount.toString() != clue) return cellList;
  return null;
},
