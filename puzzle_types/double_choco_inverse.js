...
checkRegion: function(dim, cells, shades, area) {
  ...
  if (greyAreaCoords.length != whiteAreaCoords.length) {
    return {status: "Areas of white cells and shaded cells must have the same size", errors: area};
  }
  ...
  if (Checker.compareShapes(whiteForm, shadedForm) == 0) {
    return {status: "Areas of white cells and shaded cells must have different shapes", errors: area};
  }
  return {status: "OK"};
},
