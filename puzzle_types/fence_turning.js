...
check:function(dimension, clues, data){
  ...
  while (nextNode != startNode) {
    ...
    nextEdge.used = true;
    if (prevEdge.i%2 != nextEdge.i%2) {
      // turning
      Checker.getNodeCells(prevNode,dim).forEach(cell => cells[cell.y][cell.x].count++);
    }
...
getNodeCells: function(node, dim){
  const x=node.x;
  const y=node.y;
  const i=node.i;
  var cells = [{x:x, y:y}];
