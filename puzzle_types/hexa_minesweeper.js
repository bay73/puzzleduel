...
check:function(dimension, clues, data){
  ...
  for (var y = 0; y < dim.rows + dim.cols - 1; y++) {
    ...
    for (var x = 0; x < 2*dim.cols - 1; x++) {
      if (y - x < dim.rows && x - y < dim.cols) {
        cells[y][x] = {mine: false, clue: null}
      }
    }
  }
  ...
  for (var y = 0; y < dim.rows + dim.cols - 1; y++) {
    for (var x = 0; x < 2*dim.cols - 1; x++) {
      if (y - x < dim.rows && x - y < dim.cols) {
        if (cells[y][x].clue) {
          var count = Checker.isMine(cells, x-1, y) + 
            Checker.isMine(cells, x, y-1) +
            Checker.isMine(cells, x+1, y) +
            Checker.isMine(cells, x, y+1) +
            Checker.isMine(cells, x-1, y-1) +
            Checker.isMine(cells, x+1, y+1)
