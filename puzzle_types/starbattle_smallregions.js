...
check:function(dimension, clues, data){
  ...
  var res = Checker.checkRows(cells, starCount);
  if (res.status != "OK") {
    return res;
  }
  var res = Checker.checkColumns(cells, starCount);
  if (res.status != "OK") {
    return res;
  }
...
checkRows: function(cells, requiredCount) {
  for (var y = 0; y < cells.rows; y++) {
    var starCount = 0;
    var cellList = [];
    for (var x = 0; x < cells.cols; x++) {
    ...
    if (starCount!=requiredCount) {
      return {status: "Wrong number of stars in the row", errors: cellList}
    }
  }
  return {status: "OK"};
},
checkColumns: function(cells, requiredCount) {
  for (var x = 0; x < cells.cols; x++) {
    var starCount = 0;
    var cellList = [];
    for (var y = 0; y < cells.rows; y++) {
    ...
    if (starCount!=requiredCount) {
      return {status: "Wrong number of stars in the column", errors: cellList}
    }
  }
  return {status: "OK"};
},
checkAreas: function(areas, cells) {
  for (var i = 0; i < areas.length; i++) {
    ...
    if (starCount!=1) {
      return {status: "Each outlined area should have exactly one star", errors: cellList}
    }
  }
  return {status: "OK"};
}
