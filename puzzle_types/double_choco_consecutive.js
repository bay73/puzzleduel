...
check:function(dimension, clues, data){
  ...
  res = Checker.checkNeighbouringAreas(dim, areas);
  if (res.status != "OK") {
    return res;
  }
  return {status: "OK"};
...
checkNeighbouringAreas: function(dim, areas) {
    ...
    for (var x = 0; x < dim.cols; x++) {
      if (y+1 < dim.rows) {
        if (map[y][x] != map[y+1][x]) {
          neighborSizes[map[y][x]].push(areas[map[y+1][x]].length)
          neighborSizes[map[y+1][x]].push(areas[map[y][x]].length)
        }
      }
      if (x+1 < dim.cols) {
        if (map[y][x] != map[y][x+1]) {
          neighborSizes[map[y][x]].push(areas[map[y][x+1]].length)
          neighborSizes[map[y][x+1]].push(areas[map[y][x]].length)
        }
      }
    }
  }
  for (var a=0; a<areas.length; a++) {
    areas[a].length
    if (!neighborSizes[a].includes(areas[a].length-2) && !neighborSizes[a].includes(areas[a].length+2)) {
      return {status: "The region should have a neighbouring region of consecutive size", errors: areas[a]};
    }
  }
  return {status: "OK"};
},
