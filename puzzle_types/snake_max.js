...
checkColumnClues: function(cells, clues) {
  for (var x=0; x < cells.cols; x++) {
    if (clues[x] && clues[x] != "white") {
      ...
      if (res) {
        return {status: "Wrong length of blackened fragment in the column", errors: res};
      }
    }
  }
  return {status: "OK"};
...
checkColumnClue: function(cells, clue, col) {
  ...
  if (maxCount.toString() != clue) return cellList;
  return null;
},
checkRowClues: function(cells, clues) {
  for (var y=0; y < cells.rows; y++) {
    if (clues[y] && clues[y] != "white") {
      ...
      if (res) {
        return {status: "Wrong length of blackened fragment in the row", errors: res};
      }
    }
  }
  return {status: "OK"};
