...
checkClues: function(line, clues) {
  ...
  for (var i=0;i<line.length-1;i++) {
    if (clues[line[i].y][line[i].x]) {
      if (clues[line[i].y][line[i].x]==prevClue) {
        return {
          status: "The circles along the line should alternate the color",
          errors: [util.coord(line[i].x, line[i].y)]
        };
      } else {
        prevClue = clues[line[i].y][line[i].x];
      }
    }
  }
  return {status: "OK"};
