  ...
  for (var y = 0; y < cells.rows; y++) {
    for (var x = 0; x < cells.cols; x++) {
      if (clue[y][x]!="" && clue[y][x]!="cross"){
        if (!Checker.checkKnightClue(clue[y][x], {x:x, y:y}, cells)) {
...
checkKnightClue: function(clue, position, cells) {
  var neighbour = [{y:2,x:1}, {y:1,x:2}, {y:-1,x:2}, {y:-2,x:1}, {y:-2,x:-1}, {y:-1,x:-2}, {y:1,x:-2}, {y:2,x:-1}]
  var mineCount = 0;
  for (var i=0;i<8;i++) {
